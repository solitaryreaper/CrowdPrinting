...
public class LoginService
{
	...
	public static boolean verifyLogin(String userName, String password)
	{
		...
		String loginSql = "SELECT user_id FROM " + LOGIN_TABLE
	...
	public static boolean createLogin(String loginName, String loginPassword)
	{
		...
		String loginSql = "INSERT INTO "
				+ "user"
				+ " (user_id, username, password, address1, address2, city, state, zip) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ";
		PreparedStatement preparedSql = null;
		try
		{
			preparedSql = dbConn.prepareStatement(loginSql);
	...
	public static boolean doesLoginAlreadyExist(String loginName)
	{
		...
		try
		{
			...
			// If an existing login name matches with the provided login name,
	...
	public static int getUserIDfromUsername(String userName)
	{
		...
		String loginSql = "SELECT user_id FROM " + LOGIN_TABLE + " WHERE username = ?";
		Connection dbConn = DBUtils.getDBConnection();
		PreparedStatement preparedSql = null;
		try
		{
